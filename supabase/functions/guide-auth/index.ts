...
const GUIDE_PASSWORD = Deno.env.get('GUIDE_PASSWORD')!;
const JWT_SECRET     = Deno.env.get('GUIDE_JWT_SECRET')!;

// Sign a minimal HS256 JWT valid for 30 days
async function signJwt(secret: string): Promise<string> {
  const header  = btoa(JSON.stringify({ alg: 'HS256', typ: 'JWT' })).replace(/=/g,'').replace(/\+/g,'-').replace(/\//g,'_');
  const exp     = Math.floor(Date.now() / 1000) + 30 * 24 * 3600;
  const payload = btoa(JSON.stringify({ role: 'guide', exp })).replace(/=/g,'').replace(/\+/g,'-').replace(/\//g,'_');
