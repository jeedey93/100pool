...
serve(async (req) => {
  ...
  let message: string, email: string | null;
  try {
  ...
  await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      subject: `💬 Nouveau commentaire${email ? ` — ${email}` : ''}`,
      html: `
        <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:520px;margin:0 auto;padding:32px 24px;background:#f8fafc;">
          <div style="background:white;border-radius:16px;padding:36px;box-shadow:0 2px 12px rgba(0,0,0,0.06);">
            ...
            <div style="background:#f1f5f9;border-left:4px solid #2d8a3e;border-radius:0 8px 8px 0;padding:16px 18px;margin:20px 0;color:#1e293b;font-size:0.95em;line-height:1.7;white-space:pre-wrap;">${message.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</div>
